...
export default function DashboardPage() {
  const {token,loading,error,books,setBooks,setError} =useContext(AdminContext);
 useEffect(()=>{
 ...
 <div className="min-h-screen px-5 py-4">
  ...
  {loading ? (
    ...
    <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-4 xl:grid-cols-5 gap-4">
      {books.map((book, idx) => (
        <div key={idx} className="bg-white rounded-xl shadow hover:shadow-md transition-all duration-200 overflow-hidden" >
          ...
          <div className="p-3 flex flex-col gap-1">
            ...
            <div className="flex gap-2 mt-3">
              <button className="bg-blue-500 cursor-pointer    hover:bg-blue-600 text-white text-xs px-3 py-1 rounded-full transition" >
                Update
              </button>
